...
trusted security advisories, FH Genie structured extraction and bounded ATT&CK mapping,
and Neo4j provenance.
...
Neo4j storage, FastAPI, and CLI. Independent mapping validation and final attack-path output
...
For every extracted step, Neo4j returns at most eight platform-compatible candidates ranked from
the step's evidence text. FH Genie may select only one of those candidates and one of its official
tactics, or return null IDs with low confidence. The service validates candidate membership,
platform support, tactic membership, step identity, and evidence IDs before storing `MAPS_TO`.
It does not infer post-exploitation behavior or run a separate Validation Agent.
